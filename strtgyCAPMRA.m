...
%   
function [amountOfStockInPortfolioPerTurn  indPositiveRetsAfterProtectFinal indPositiveRetsAboveVaR] = strtgyCAPMRA(forecast_data, hist_data, today_prices, Rf, TOTAL_CASH_MONEY,cash_history)
...
%      forecast_data = abs(forecast_data);
    ...
    for hj = 1: k
    for hkj = 1:v
        if forecast_data(hj,hkj) < 0.01
            forecast_data(hj,hkj) = 0.01;
        end
    end
end
    %---------------
 expForecastRet = (forecast_data' -today_prices)./today_prices;
   ...
   [RetSeries] = tick2ret((hist_data)');
     ...
     %now let's try Zhenja suggestion
%      expForecastRet = (forecast_data' -today_prices)./today_prices;
