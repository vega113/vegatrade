...
%   
function [amountOfStockInPortfolioPerTurn  indPositiveRetsAfterProtectFinal] = strtgyCAPM10(forecast_data, hist_data, today_prices, Rf,TOTAL_CASH_MONEY,cash_history)
...
[VaR deviation isVaRAcceptedLRuc indPositiveRetsAboveVaR] = filterByVaR(hist_data,today_prices,MAX_VALUE_AT_RISK_PERCENT,VaR_CLEVEL,1);
   ...
   [RetSeries] = tick2ret((hist_data(indPositiveRetsAboveVaR,:))');
     ...
     expForecastRet = (forecast_data' -today_prices)./today_prices;
     [PortRisk, PortReturn, PortWts] = frontcon((expForecastRet(indPositiveRetsAboveVaR))',ExpCovariance,10);
